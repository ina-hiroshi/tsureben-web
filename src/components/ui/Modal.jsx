...
export default function Modal({ open, onClose, title, children, fullScreenMobile = true }) {
  if (!open) return null;

  const panelClass = fullScreenMobile
    ? 'fixed inset-0 z-[100] flex flex-col bg-tsure-surface sm:inset-auto sm:relative sm:max-w-lg sm:mx-auto sm:my-8 sm:rounded-2xl sm:max-h-[90vh]'
    : 'relative w-full max-w-lg bg-tsure-surface rounded-2xl max-h-[90vh] flex flex-col';
