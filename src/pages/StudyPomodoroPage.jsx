...
export default function StudyPomodoroPage() {
    ...
    const handleStart = async () => {
        if (!selectedPlan) {
            toast.warning('この時間の学習計画が登録されていません。先に登録してください。');
            return;
        }
    ...
    const handleFinish = async () => {
        ...
        if (!startTime || !currentUser || !currentUser.email) {
            try {
                ...
                toast.success('再ログインに成功しました。記録を続けます。');
                ...
                while (true) {
                    ...
                    if (manual === null) {
                        toast.info('キャンセルされました。記録は保存されませんでした。');
                        return;
                    }
                    ...
                    toast.warning('入力が不正です。もう一度、1〜1000の数値で入力してください。');
                }

                toast.success(`${manualMinutes}分として記録しました。`);
                return;
            }
        }
        ...
        if (!Number.isFinite(minutes) || minutes <= 0 || minutes > 1000) {
            ...
            toast.info(`${minutes}分として手動記録します`);
        }
        ...
        toast.success(`${minutes}分の学習を記録しました`);
    };
    ...
    }, [user, selectedGrade, selectedClass, isTeacher]);
